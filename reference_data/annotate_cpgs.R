...
rm(list=ls())
setwd("~/Documents/UCSF_year1/Zaitlen-rotation1/")

######################### reference data ###############################

illumina = read.csv("HumanMethylation450_15017482_v1-2.csv", header=TRUE) # loads in illumina info with probe name and location
obs_cpg = read.table("cpgs.txt", header=TRUE) # a list of cpgs observed in my ALS dataset 
merged_reference = read.table("matrix_merged.txt", header=TRUE) # my compiled reference cpg probes 

######################### clean data ###############################

illumina_obs = illumina[illumina$IlmnID %in% obs_cpg$x, ] # remove any probes from illumina data not seen in ALS data
obs_cpg_annot =  subset(merged_reference, merged_reference$X.Sample_geo_accession %in% illumina_obs$IlmnID) # some data not with probe # are in my matrix, so remove those too (mostly points not in a majority of sources) 
